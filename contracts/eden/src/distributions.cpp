...
namespace eden
{
   ...
   bool setup_distribution(eosio::name contract, accounts& accounts, eosio::block_timestamp init)
   {
      ...
      if (iter == distribution_tb.begin())
      {
         ...
         {
            eosio::block_timestamp seeding_start = eosio::time_point(init) - eosio::seconds(22 * 60 * 60);
            push_event(distribution_event_schedule{seeding_start}, contract);
            distribution_tb.emplace(contract,
                                    [&](auto& row) { row.value = next_distribution{seeding_start}; });
         }
         else
         {
            return false;
